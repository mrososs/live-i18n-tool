...
import { JsonTranslateLoader } from './i18n/translate-loader';

// Emit invisible key markers from the translate pipe so the inspector can
// recover the exact key per element — even when two strings render the same
// text (dev-only; a no-op in production builds).
